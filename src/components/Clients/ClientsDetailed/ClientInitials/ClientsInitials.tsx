...
export const ClientsInitials = ({ client }: Props) => {
  ...
  return (
    <div className={`${styles.clientsInitialsDiv}`}>
      <div className="flex items-center justify-between mt-15 max-w-[985px] w-full ml-25">
        <h2 className="text-black text-[30px] font-sans">{client.name}</h2>
        <div
          onClick={() => router.back()}
          className="flex items-center gap-2 text-sm text-gray-500 hover:underline hover:cursor-pointer"
        >
          <IconButton
            className=""
            icon={backIcon}
      ...
      <div className="flex items-center mt-6 ml-25 gap-10">
        {columns
