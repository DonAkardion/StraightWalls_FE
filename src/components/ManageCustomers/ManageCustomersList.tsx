...
export const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjEsImxvZ2luIjoiYWRtaW4iLCJyb2xlIjoiYWRtaW4iLCJmdWxsTmFtZSI6ItCh0LjRgdGC0LXQvNC90LjQuSDQkNC00LzRltC90ZbRgdGC0YDQsNGC0L7RgCIsImlhdCI6MTc1NTYwMTI3MywiZXhwIjoxNzU1Njg3NjczfQ.tlgv5y1aJ9XPKKlYr_wmn3CQ28v9ydDDlsQ81HnGClQ"
...
export const ManageCustomers = ({ onDelete }: ManageCustomerProps) => {
    ...
    const handleSavePassword = async (newPassword: string) => {
        ...
        setEditingId(null);
        setUsers(prev => prev.map(u => u.id === editingId ? {...u, password: "********"} : u));
    }
    ...
    return (
        <div>
            <Table<ManageCustomersProps>
                ...
                columns={[
                    { key: "login", label: "Логін користувача", tooltip: (user) => `Логін користувача: ${user.login}` },
                    { key: "password", label: "Пароль користувача"},
                    { key: "role", label: "Роль користувача", tooltip: (user) => `Роль користувача: ${user.role}` },
                    { key: "full_name", label: "Ім'я користувача", tooltip: (user) => `Повне ім'я користувача: ${user.full_name}` }
                ]}
                showIndex={true}
            ...
            {isCustomerOpenModal && (
                <AddCustomerModal onSubmit={handleCustomerAdd} onClose={() => setIsCustomerOpenModal(false)}/>
            )}
        </div>
    );
}
